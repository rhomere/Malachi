MALACHIAPP.controller('EligibilityController', ['$rootScope', '$scope', '$location', '$timeout', '$modal', '$stateParams', '$state', '$filter', 'settings', 'contractService', 'accountService', 'toolsService', 'settingsService', function ($rootScope, $scope, $location, $timeout, $modal, $stateParams, $state, $filter, settings, contractService, accountService, toolsService, settingsService) {
    ...
    contractService.getEligibilities($scope.Contract.Id).then(function (result) {
        if (result.data.Result.Success) {
            $scope.Eligibilities = result.data.Eligibilities;
            $scope.TotalEligibilities = result.data.Eligibilities;
    ...
    $scope.searchEligibilities = function (contractId, codeNumber, pageNumber, display) {
        contractService.searchEligibilities(contractId, codeNumber, pageNumber, display).then(function (result) {
            if (result.data) {
                $scope.Eligibilities = result.data.Eligibilities;
            }
            else {
    ...
    $scope.copyConditionGroup = function (conditionGroup, eligibility) {
        contractService.copyEligibilityConditionGroup(conditionGroup.Id).then(function (result) {
            eligibility.Conditions.push(result.data);
        }, function (error) {
            $scope.Errors = ['An unexpected error has occured. Please refresh the page.'];
        });
    };
    ...
    $scope.copyEligibility = function (eligibility) {
        var modalInstance = $modal.open({
            templateUrl: 'copyEligibility.html',
            controller: 'copyEligibilityCtrl',
            backdrop: 'static',
            size: 'md',
            resolve: {
        ...
        modalInstance.result.then(function (result) {
            if (result !== 'cancel') {
                var eligibilityId = eligibility.Id;
                var fromContractId = eligibility.ContractId;
                var toContractId = result.contract.Id;

                contractService.copyEligibility(eligibilityId, fromContractId, toContractId).then(function (result) {
                    if (result.data.Result.Success) {
                        var copiedEligibility = result.data.Eligibility;
                        if (copiedEligibility != null) {
                            if (fromContractId === toContractId) {
                                $scope.Eligibilities.push(copiedEligibility);
                            }
                        }
                    } else {
                        $scope.Errors = result.data.Result.Errors;
    ...
    $scope.deleteEligibility = function (eligibility) {
        contractService.deleteEligibility(eligibility.Id).then(function (result) {
            if (result.data.Result.Success) {
    ...
    $scope.saveEligibility = function () {
        ...
        contractService.updateEligibility($scope.Eligibility).then(function (result) {
            if (result.data.Result.Success) {
                if (isNew) {
                    $scope.Eligibilities.push(result.data.Eligibility);
                }
                else {
                    for (var i = 0; i < $scope.Eligibilities.length; i++) {
                        if ($scope.Eligibilities[i].Id == result.data.Eligibility.Id) {
                            $scope.Eligibilities[i] = result.data.Eligibility;
                        }
                    }
                }
                // Clean up
...
MALACHIAPP.controller('copyEligibilityCtrl', ['$rootScope', '$http', '$scope', '$location', '$stateParams', '$ocLazyLoad', 'notificationsHub', '$modalInstance', 'contract', 'contracts', function ($rootScope, $http, $scope, $location, $stateParams, $ocLazyLoad, notificationsHub, $modalInstance, contract, contracts) {
