...
MALACHIAPP.controller('RiskBordereauxController', ['$rootScope', '$scope', '$http', '$timeout', '$location', '$modal', 'authService', 'policyService', 'notificationsHub', 'ngAuthSettings', 'localStorageService', function ($rootScope, $scope, $http, $timeout, $location, $modal, authService, policyService, notificationsHub, ngAuthSettings, localStorageService) {
  ...
  $scope.RiskBordereaux = { ReportType: 0 };
  $scope.RiskTypes = [{ id: 0, name: "Risk" }, { id: 1, name: "Risk & Premium" }];
  $scope.brokers = [];
  $scope.markets = [];
  $scope.errorList = [];
  ...
  $scope.selectStates = function (selectAll) {
    if (selectAll == true) {
      $scope.RiskBordereaux.States = [];

      $scope.states.forEach(function (state) {
        $scope.RiskBordereaux.States.push(state.Code);
      });
  ...
  $scope.selectMarkets = function (selectAll) {
    if (selectAll == true) {
      $scope.RiskBordereaux.Markets = [];

      $scope.markets.forEach(function (market) {
        $scope.RiskBordereaux.Markets.push(market.Contract);
      });
  ...
  $scope.getMarkets = function () {
    if ($scope.RiskBordereaux.Brokers.length == 0) return;
    // Reset the selected contracts
    $scope.RiskBordereaux.Markets = [];

    Metronic.blockUI({ animate: true, overlayColor: 'none' });
    var params = { Brokers: $scope.RiskBordereaux.Brokers };
  ...
  $scope.validateRiskBordereaux = function () {

    var riskbrdx = $scope.RiskBordereaux;

    if (riskbrdx.States == undefined || riskbrdx.States.length == 0) {
      $scope.errorList.push('Please select at least one state in order to run the report');
    }

    if (riskbrdx.Markets == undefined || riskbrdx.Markets.length == 0) {
  ...
  $scope.requestReport = function () {
    $scope.errorList = [];
    $scope.validateRiskBordereaux();
    if ($scope.errorList.length > 0) return;

    var params = { BordereauxRequest: $scope.RiskBordereaux };

    Metronic.blockUI({ animate: true, overlayColor: "none" });
    $http.post(window.reportsAPI + "api/export/RequestRiskBordereaux", params).then(result => {
      Metronic.unblockUI();
  ...
  $scope.downloadReport = function () {
    ...
    xhr.open('POST', window.reportsAPI + 'api/export/RiskBordereaux', true);
    xhr.responseType = 'arraybuffer';
    xhr.onload = function () {
      Metronic.unblockUI();
      if (this.status === 200) {
        var filename = "RiskBordereaux.xlsx";
    ...
    xhr.send(JSON.stringify({ BordereauxRequest: $scope.RiskBordereaux }));
  }
