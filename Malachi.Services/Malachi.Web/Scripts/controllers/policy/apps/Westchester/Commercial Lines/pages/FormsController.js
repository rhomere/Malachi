...
MALACHIAPP.controller('test_Commercial_Lines_FormsController', ['$rootScope', '$scope', '$location', '$stateParams', '$ocLazyLoad', '$modal', 'authService', 'notificationsHub', 'settings', 'policyService', 'toolsService', 'ngAuthSettings', 'localStorageService', 'customPackageService', '$filter', function ($rootScope, $scope, $location, $stateParams, $ocLazyLoad, $modal, authService, notificationsHub, settings, policyService, toolsService, ngAuthSettings, localStorageService, customPackageService, $filter) {
  ...
  setTimeout(function () {
    ...
    var selector = "li[heading='" + getRiskCompanyName(focusedRiskCompany) + "']";
  ...
  $scope.istest = function (riskCompanyId) {
    return riskCompanyId == 'b831d0f1-9c3b-4b5f-986a-b53273051556' || riskCompanyId == '9f2a41e8-a05d-41ed-a224-b179c82e5f50';
  }
  ...
  function getRiskCompanyName(riskCompany) {
    return $scope.isCustomPackageRiskCompany(riskCompany) ? $scope.getCustomPackageRiskCompanyName(riskCompany.Id) : riskCompany.Name;
  };
  $scope.getRiskCompanyName = getRiskCompanyName;
  ...
  $scope.getCustomPackageCoverageForms = function (coverageName, riskCompanyId) {
    var forms = $scope.parent.Policy.CurrentVersion.Forms;
    var riskCompanies = $scope.parent.RiskCompanies;

    var filteredForms = customPackageService.getCoverageForms(forms, riskCompanies, coverageName, riskCompanyId);
    var orderedForms = $scope.filterForms(null, filteredForms);
  ...
  $scope.filterForms = function (riskCompanyId, forms) {
    var filteredRiskCompanies = [];

    if (riskCompanyId != null) {
      filteredRiskCompanies = $scope.parent.Policy.CurrentVersion.Forms.filter(function (x) {
        return !x.IsRemoved && x.RiskCompanyId == riskCompanyId;
      });
    }
    else if (forms != null) {
      filteredRiskCompanies = forms.filter(function (x) {
        return !x.IsRemoved;
      });
    }

    var orderedRiskCompanies = $filter('orderBy')(filteredRiskCompanies, ['-SuggestToDelete', '-RequireUserInput', 'Index', 'Number']);
    return orderedRiskCompanies;
  };

  $scope.addForm = function (riskCompany) {
    var modalInstance = $modal.open({
      templateUrl: 'formAddModelContent.html',
      controller: 'test_Commercial_Lines_formAddCtrl',
      backdrop: 'static',
      size: 'lg',
      resolve: {
        forms: function () {
  ...
  $scope.editForm = function (form) {
    var modalInstance = $modal.open({
      ...
      controller: 'test_Commercial_Lines_formUpdateCtrl',
      backdrop: 'static',
      size: 'lg',
      resolve: {
        policy: function () {
      ...
      if (location != 'cancel') {
        policyService.updateForm($scope.PolicyId, form).then(function (result) {
          if (result.data.Result.Success) {
            ...
            policy.ValidateQuote = result.data.ValidateQuote;
            policy.ValidateBind = result.data.ValidateBind;
            policy.ValidateQuoteByCompany = result.data.ValidateQuoteByCompany;
            policy.ValidateBindByCompany = result.data.ValidateBindByCompany;
  ...
  $scope.attachFormToQuoteDocument = function (form) {
    ...
    policyService.attachFormToQuoteDocument(policyId, form).then(function (result) {
      if (result.data.Result.Success) {
        ...
        var isAttached = form.IsAttachedToQuoteDocument;
...
MALACHIAPP.controller('test_Commercial_Lines_formUpdateCtrl', ['$rootScope', '$http', '$scope', '$location', '$stateParams', '$ocLazyLoad', 'notificationsHub', '$modalInstance', 'settings', 'policyService', 'toolsService', 'policy', 'form', 'locations', 'parent', function ($rootScope, $http, $scope, $location, $stateParams, $ocLazyLoad, notificationsHub, $modalInstance, settings, policyService, toolsService, policy, form, locations, parent) {
  ...
  function setupOptionItems() {
    var optionItemFields = $scope.Form.Fields.filter(x => x.OptionItems != null && x.OptionItems.length > 0);

    for (let field of optionItemFields) {
      var optionItems = $.extend(true, [], field.OptionItems);
      optionItems.sort((a, b) => a.Index - b.Index);
      $scope.OptionItems[field.Id] = optionItems;
    }

    for (let table of $scope.Form.Tables) {
      for (let row of table.Rows) {
        optionItemFields = row.Fields.filter(x => x.OptionItems != null && x.OptionItems.length > 0);
