...
MALACHIAPP.controller('test_Commercial_Lines_BindEndorsementController', ['authService', '$rootScope', '$scope', '$location', '$stateParams', '$ocLazyLoad', '$modal', 'notificationsHub', 'settings', 'policyService', 'ngAuthSettings', 'localStorageService', function (authService, $rootScope, $scope, $location, $stateParams, $ocLazyLoad, $modal, notificationsHub, settings, policyService, ngAuthSettings, localStorageService) {
    ...
    $scope.canAdjustPremium = $.inArray("Endorsement Premium Update", authService.authentication.roles) > -1;
    $scope.isRliPortalApp = $scope.AppId.toLowerCase() === 'dd8ff3ae-980c-4ee3-bc7e-1f20d92e327e';
    ...
    $scope.confirmBindEndorsementContact = function () {
      var modalInstance = $modal.open({
        ...
        controller: 'test_Commercial_Line_confirmBindContactCtrl',
        backdrop: 'static',
        resolve: {
          Parent: function () {
            return $scope.parent;
    ...
    $scope.updateEndorsementPremium = function () {
        ...
        /* Make sure user adjusted amount fields are not set to null but instead defaults to the developed amount */
        for (var i = 0; i < $scope.EndorsementRecord.PremiumBreakdowns.length; i++) {
          var premiumBreakdown = $scope.EndorsementRecord.PremiumBreakdowns[i];
          if (!premiumBreakdown.Amount)
            premiumBreakdown.Amount = premiumBreakdown.DevelopedAmount;
        }
    ...
    $scope.isUserAdjustedPremium = function() {
        for (let classCode of $scope.Policy.CurrentVersion.ClassCodes) {
            for (let result of classCode.ClassCodeRatingResults) {
                var input = classCode.ClassCodeRatingInputs.find(function (x) { return x.ContractId == result.ContractId; });
                if (input.IsUserEnteredMinimumPremium && result.TotalPremium != result.TotalDevelopedPremium) {
                  return true;
                }
            }
        }
    ...
    $scope.addEndorsementFee = function () {
        var modalInstance = $modal.open({
            ...
            controller: 'test_Commercial_Lines_addEndorsementFee',
            backdrop: 'static',
            resolve: {
    ...
    $scope.bindEndorsement = function (emailToAgent) {
        $scope.Errors = [];
        policyService.bindGeneralEndorsement($scope.PolicyId, emailToAgent, null, null, $scope.EndorsementRecord, $.grep($scope.EndorsementRecord.PremiumBreakdowns, function (x) { return x.IsAddedByUser == true; }), null, null, $scope.parent.Policy.ReferralNumber).then(function (result) {
                if (result.data.Result.Success) {
    ...
    $scope.downloadEndorsement = function () {
        ...
        if ($scope.parent.canModify())
            xhr.send(JSON.stringify({
                ...
                EndorsementRecord: $scope.EndorsementRecord,
                CustomFees: $.grep($scope.EndorsementRecord.PremiumBreakdowns, function (x) { return x.IsAddedByUser == true; })
            }));
        else
            xhr.send(JSON.stringify({ PolicyId: $scope.PolicyId }));
    }

    $scope.isNullOrEmpty = function (text) {
        return typeof (text) != 'string' || text === '';
    }
}]);

MALACHIAPP.controller('test_Commercial_Lines_addEndorsementFee', ['$rootScope', '$scope', '$location', '$stateParams', '$ocLazyLoad', 'notificationsHub', '$modalInstance', '$http', 'settings', 'policyService', 'toolsService', 'endorsementRecord', function ($rootScope, $scope, $location, $stateParams, $ocLazyLoad, notificationsHub, $modalInstance, $http, settings, policyService, toolsService, endorsementRecord) {
